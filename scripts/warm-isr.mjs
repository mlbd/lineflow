...
const BASIC_AUTH =
  WP_USER && WP_PASS ? 'Basic ' + Buffer.from(`${WP_USER}:${WP_PASS}`).toString('base64') : '';
...
if (!BASE || !WP) {
  console.error(
    'ERROR: Set PUBLIC_BASE_URL (or NEXT_PUBLIC_SITE_URL) and WP_SITE_URL (or NEXT_PUBLIC_WP_SITE_URL).'
  );
  process.exit(1);
}
...
async function listSlugs() {
  ...
  console.log(
    `Loaded ${slugs.length} total slugs; processing ${slice.length} (offset=${OFFSET}${LIMIT != null ? `, limit=${LIMIT}` : ''}).`
  );
  return slice;
}
