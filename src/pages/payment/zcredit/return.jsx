    ...
    const check = async () => {
      try {
        if (transactionUniqueId) {
          const r = await fetch(
            `/api/payments/zcredit/get-status?transactionUniqueId=${encodeURIComponent(transactionUniqueId)}`
          );
  ...
  return (
    <div className="max-w-xl mx-auto p-8">
      ...
      <div className="bg-white border rounded p-4 text-sm space-y-2">
        ...
        <div>
          <strong>מזהה עסקה:</strong> {result?.tx?.TransactionUniqueId || '-'}
        </div>
        <div>
          <strong>סכום:</strong> {result?.tx?.TransactionSum || '-'}
        </div>
        <div>
          <strong>אסמכתא:</strong> {result?.tx?.ApprovalNumber || '-'}
        </div>
      </div>
