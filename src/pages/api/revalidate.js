...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (q.slugs) {
      ...
      else String(q.slugs).split(',').forEach(v => push(slugs, v));
    }
    if (q.path) push(paths, q.path);
    if (q.paths) {
      if (Array.isArray(q.paths)) q.paths.forEach(v => push(paths, v));
      else String(q.paths).split(',').forEach(v => push(paths, v));
    }
    ...
    const toPaths = Array.from(new Set([
      ...paths.map(p => `/${String(p).replace(/^\/+/, '')}`),
      ...slugs.map(s => `/${String(s).replace(/^\/+/, '')}`),
    ]));

    if (toPaths.length === 0) {
      return res.status(400).json({ ok: false, error: 'Provide ?slug=, ?slugs=, ?path=, ?paths= or JSON body { slugs|paths }' });
    }
