...
export default function ShippingOptions({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-md space-y-6">
      <h3 className="text-lg font-bold text-gray-900 text-center border-b mb-4 border-gray-200 py-6">אפשרויות משלוח</h3>
          ...
          <div className="flex flex-col gap-3">
            {shippingOptions.map((option) => (
              <label
                ...
                `}
              >
                <span className={`absolute right-0 h-[100%] z-0 w-[30px] bg-blue-700
                  ${selectedShipping?.id === option.id
                    ? 'bg-blue-600'
                    : 'bg-gray-200'}`}
                ></span>
                ...
                <div className="flex flex-col flex-grow">
                  <div className="flex justify-between items-center w-full">
                    <span className="font-semibold text-sm text-gray-900">{option.label} {option.cost === 0 ? 'חינם' : `${option.cost} ₪`}</span>
                  </div>
            ...
            {shippingOptions.length === 0 && (
              <div className="text-gray-400 text-center py-4">אין אפשרויות משלוח זמינות לסל הנוכחי.</div>
            )}
          </div>
